      ...
      setOrdersFromApi: (apiOrders) =>
        set((state) => {
          ...
          const merged: DietitianOrder[] = apiOrders.map((o) => {
            ...
            const lineQty = Math.max(0, Math.floor(Number(o.quantity) || 0))
            const skuPerLine =
              o.salesKit?.quantity != null && Number.isFinite(Number(o.salesKit.quantity))
                ? Math.max(1, Math.floor(Number(o.salesKit.quantity)))
                : null
            /** Zimmetlenecek fiziksel kit adedi: satır adedi × ürün kartındaki kit adedi (salesKit.quantity) */
            const fulfillKitCount = skuPerLine != null ? lineQty * skuPerLine : lineQty
            return {
              id,
